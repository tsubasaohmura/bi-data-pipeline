...
def hana_to_ch(*, hana_conn_id, hana_sql_file, ch_conn_id, ch_target_table, transform_func=None, truncate=False,
               ...
               max_fetch_rows=100_000, **context):
...
"""
    ...
    try:    
        ...
        #if monthly_job:
        #    # Remove previous month's record
        #    logger.info(f"Refreshing ClickHouse table {ch_target_table} previous month...")
        #    ch.run(f"ALTER TABLE {ch_target_table} DELETE WHERE calmonth = '{target_month_previous}';")
        #    logger.info(f"Refreshed previous month's {ch_target_table} table successfuly.")
            ...
            if transaction:
                ...
                for record in result:
                    ...
                    for column, value in zip(record.column_names, record.column_values):
                        ...
                        if column in ("CALDAY", "BUDAT", "BW_CALDAY_WEEK", "CALDAY_prevyear", "CALDAY_WEEK_prevyear", "0CALDAY", "ZJDATE__ZJCPDATE") \
        ...
        if master:
            ...
            if master == 'product':
                ...
                cols_master = ['product_code', 'product_name', 'product_type', 'product_uuid', 'product_local_name',
                                    ...
                                    'is_active', 'category_name', 'category_local_name', 'images']
                table_master = DataFrame(index = [], columns = cols_master)
